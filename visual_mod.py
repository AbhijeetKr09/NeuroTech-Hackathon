...
class SerialPlotter(pg.GraphicsLayoutWidget):
    def __init__(self, channels, data_length, port, baudrate=115200, timeout=1, bandpass_freq=[4, 8], notch_freq=50, sample_rate=250):
        super().__init__()
    ...
    def update(self, data):
        if data != '\r\n' and data != '' and data != '\n':
            ...
            data_focused.append(data)

            if time.time() - start_time > 10:
                for data in data_focused:
                    if float(data) > 4.0 and  float(data) < 8.0:
                        data_focused_2.append(data)
                with open('unfocused_data.json', "w") as f:
                    json.dump(data_focused_2, f)
                print(data_focused_2)
...
viewer = SerialPlotter(channels, 750, 'COM10')
